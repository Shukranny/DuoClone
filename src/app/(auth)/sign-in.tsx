...
export default function SignInScreen() {
  ...
  const handleSignIn = async () => {
    ...
    if (!email.trim() || !password.trim()) {
      Alert.alert("Invalid Input", "Please enter both your email and password.");
      return;
    }
    ...
    if (error) {
      ...
      Alert.alert(
        "Sign In Failed",
        error.longMessage || error.message || "Check your email and password and try again."
      );
      return;
    }
    ...
    if (signIn.status === "needs_client_trust") {
      // Default second factor is an email verification code -- show the modal.
      ...
      if (emailFactor) {
        try {
          ...
          Alert.alert(
            "Error",
            error?.longMessage || error?.message || "Failed to send verification code. Please try again."
          );
        }
        return;
      }
    ...
    if (signIn.status === "needs_second_factor") {
      // MFA is configured on this account -- for this teaching project we
      // fall back to the email-code modal as the simplest path.
      try {
        await signIn.mfa.sendEmailCode();
        setShowModal(true);
      } catch (error: any) {
  ...
  // Called by VerificationModal when the second-factor email code is entered.
